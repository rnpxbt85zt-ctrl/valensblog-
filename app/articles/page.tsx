    ...
    <div style={{ padding: '4rem 0' }}>
      <div className="container">
        ...
        <div style={{ display: 'flex', gap: '.75rem', marginBottom: '2.5rem', flexWrap: 'wrap' }}>
          <button onClick={() => setCategory('all')} className={`btn ${category === 'all' ? 'btn-primary' : 'btn-outline'}`} style={{ padding: '.4rem 1rem', fontSize: '.85rem' }}>
            {t.allCategories}
          </button>
          {CATEGORIES.map((cat) => (
            <button key={cat} onClick={() => setCategory(cat)} className={`btn ${category === cat ? 'btn-primary' : 'btn-outline'}`} style={{ padding: '.4rem 1rem', fontSize: '.85rem' }}>
              {cat}
            </button>
          ))}
        </div>
            ...
            {articles.map((article) => (
              <Link key={article.id} href={`/article/${article.slug}`} className="card" style={{ display: 'block' }}>
                ...
                <div style={{ padding: '1.25rem' }}>
                  {article.category && <span className="badge badge-blue" style={{ marginBottom: '.75rem', display: 'inline-block' }}>{article.category}</span>}
                  <h2 style={{ fontFamily: 'var(--font)', fontWeight: 700, fontSize: '1.15rem', marginBottom: '.5rem', lineHeight: 1.4 }}>{article.title}</h2>
                  {article.summary && <p style={{ color: 'var(--text2)', fontSize: '.9rem', marginBottom: '1rem', lineHeight: 1.6, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{article.summary}</p>}
